...
void Installer::installNewServerConfigFile(string fileLocation, bool skipFileCheck, string flags, string serverFile) {
	if (fs::is_regular_file(fileLocation) && !skipFileCheck) {
		...
		outConf << "name=" << std::regex_replace(fileLocation, std::regex("\\..*", std::regex_constants::optimize), "") << endl << "path=" << fs::current_path().string() << endl << "exec=java" << endl << "flags=-jar -Xmx4G -Xms4G " + flags << endl << "file=" + serverFile << endl << "command=" << endl << "method=new" << endl << "device=" << endl << "restartmins=" << endl;
