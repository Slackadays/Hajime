...
void initialHajimeSetup(string confFile, string serversFile, string serverFile, string sysdService) {
    ...
    if (logObj->getYN()) {
      for (bool skipFileCheck = false; true;) {
        try {
          installer.installDefaultHajConfFile(confFile, skipFileCheck);
          break;
        }
        catch (int i) {
          if (i == 0) {
            ...
            logObj->out("Hajime config file not created", Error);
    ...
    if (logObj->getYN()) {
      for (bool skipFileCheck = false; true;) {
        try {
          installer.installDefaultServersFile(serversFile, skipFileCheck);
          break;
        }
        catch (int i) {
          if (i == 0) {
            logObj->out("Foud an existing servers file", Warning);
            logObj->out("Do you want to install a new one?", Question);
            if (logObj->getYN()) {
              skipFileCheck = true;
            } else {
            ...
            logObj->out("Servers file not created", Error);
            logObj->out("Do you want to try creating one again?", Question);
            if (!logObj->getYN()) {
              break;
            }
    ...
    if (logObj->getYN()) {
      for (bool skipFileCheck, useFlags, askedForFlags = false; true;) {
        try {
          ...
          if (useFlags) {
            installer.installDefaultServerConfFile(serverFile, skipFileCheck, optFlags);
          } else {
            installer.installDefaultServerConfFile(serverFile, skipFileCheck);
          }
          installedS = true;
          break;
        }
